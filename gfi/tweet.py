...
"""
Whenever the script is run, starting from the most addition recent entry in
`data/repositories.toml`, we check if we have tweeted about it in the past
`GoodFirstTwitter.TWEET_AGAIN` days. If yes, we move onto the ~next~ previous
entry.

We use https://jsonbin.io/ for storing a persistent record of our tweets.

In a single session, we tweet at most `GoodFirstTwitter.TWEET_LIMIT` times.
"""
